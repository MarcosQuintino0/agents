...
Antes de caçar endpoints no backend, verifique se existe `.ai/agents/mapeamento/mapeamento-api.md`.
Se existir:

- grep pela API solicitada (ex.: `## alegacao-ans`);
- use os **ponteiros** (caminhos relativos dos arquivos) para ir direto ao controller/DTO/service;
- leia os **detalhes** (payloads, regras, mensagens) do **código real** — o mapeamento só aponta
  ONDE, não extrai O QUÊ. Se um ponteiro apontar para um arquivo que não existe mais, caça inline.

Se NÃO existir, faça a descoberta inline usando as fontes abaixo (como hoje):

1. codigo-fonte do backend;
2. OpenAPI ou Swagger;
3. colecao Insomnia ou Postman;
4. respostas reais observadas.
