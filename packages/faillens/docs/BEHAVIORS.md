...
O toolbar de detalhe possui, nesta ordem, `Diagnóstico`/`Resultado`, `Reprodução` e `Chamado`, com semântica ARIA, navegação por setas/Home/End e transição visual curta. A comparação `Esperado vs. recebido` mantém dois painéis lado a lado, ações de copiar/ampliar por ícone e destaque localizado no payload divergente. A timeline de chamadas fica dentro do diagnóstico como accordion fechado por padrão, com filtro `Principais`/`Todas`, status HTTP textual, redirects e barra relativa de duração (`accent` para chamada normal, `error` para a chamada falha). Drawers de validações/provas deslizam pela direita e mantêm o contexto da tela. A terceira aba compõe em memória, sem alterar o schema, as doze seções aprovadas: título, contexto, BDD, resultados atual/esperado, request, response, comparação, falha, cURL, evidência visual e rastreabilidade.
