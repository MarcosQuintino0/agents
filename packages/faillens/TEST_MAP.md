...
- Aba `Chamado` (heading "Criar chamado") contém as doze seções aprovadas e mantém fallback quando não há screenshot

**Contrato visual (não regredir a aparência) — `visual-styling.test.js`:**
- Paleta Midnight Pro (dark) / Clean Light (light): acento azul-petróleo `--accent` (não mais violeta), `--green`/`--red`/`--amber`/`--info` com `-soft`/`-line` derivados do mesmo RGB do token base. Ver `docs/DESIGN_SYSTEM.md` seção 1.
- Tipografia Inter (`--font-main`) para UI, JetBrains Mono (`--font-code`) só para código/dados — ver seção 2 do design system.
- Cores de método usam tokens semânticos (`get` → `var(--green)`, `put/patch` → `var(--amber)`, demais → `var(--info)`), exceto OPTIONS/HEAD que mantêm cor própria.
- Coloração ciente de estado: `.comparison-card.received.passed/.failed`, `.overview-strip` + `.status-chip.ok/.bad`, `.failure-banner.passed`, `.match-note`
- Shell: `.report-shell`/`.sidebar`/`.main` com altura 100% e scroll interno independente; sidebar colapsável (`sidebar-collapsed`, `#sidebar-collapse`)
- Diagnóstico rico exposto na UI: `proofsHtml`/`allProofsHtml` (a partir de `test.facts`) e `ruleImpactHtml` (a partir de `test.ruleRefs` + `contracts` + `diagnosis.suggestedAction`) — dado que já existia no modelo mas antes só alimentava o texto do chamado
...
- Cenário pulado: `skipCardHtml`, card único sem overview nem abas
...
- Fontes Inter + JetBrains Mono embutidas em base64 (sem links externos) — `src/templates/embeddedFont.ts`
- Sequência/timeline: barra relativa de duração alinhada ao protótipo (`accent` para chamada normal, `error` para a chamada falha, `faint` sem resposta), status HTTP no texto da linha (`statusBarClass`, `.request-bar.s2/s3/s45/snone/.bad`), método em texto mono de acento, saltos de redirect inline (`seq-hop`) e tags por chamada (`request-tags`/`meta-chip-tiny`), limite 10 + "mostrar mais"
- Comparação Esperado/Recebido: sempre em 2 colunas com scroll horizontal em viewport estreita, ações de copiar/ampliar agrupadas à direita e painel recebido divergente com tingimento sutil, sem cabeçalho vermelho pesado.
- Motion visual: tabs com transição e painel com `panel-fade`; timeline abre por `max-height`/opacidade; drawer/modal entram com overlay e `translate`/scale, preservando Escape/fechar/cópia.
- Menu lateral: contadores `✕`/`✓` (`spec-counts`), colapso de suite (`data-spec-toggle`) e dos que passaram (`data-passed-toggle`, rótulo "+N Aprovados ↓")
