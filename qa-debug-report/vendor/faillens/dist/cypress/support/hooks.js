...
exports.getCurrentTestContext = getCurrentTestContext;
exports.installFailLensHooks = installFailLensHooks;
let currentContext;
let capturedAssertions = [];
let assertionCounter = 0;
function commandAttributes(command) {
    if (!command)
        return {};
...
function printable(value) {
    ...
    if (typeof value === "function")
        return value.name ? `[callback ${value.name}]` : "[callback]";
    try {
...
function cleanAssertionMessage(value) {
    return printable(value).replace(/\*\*/g, "").replace(/\s+/g, " ").trim();
}
function assertionTitle(args) {
    if (!args.length)
        return "Assertion Cypress";
    if (typeof args[0] === "function")
        return "Validação personalizada";
...
function installAssertionCapture() {
    ...
    Cypress.on("skipped:command:end", (raw) => {
        const command = commandAttributes(raw);
        const assertion = findAssertion(command.id);
        if (assertion) {
            assertion.ended = true;
    ...
    Cypress.on("log:added", (raw) => {
        if (!currentContext || String(raw?.name || "").toLowerCase() !== "assert")
            return;
        ...
        if (raw?.message)
            assertion.title = cleanAssertionMessage(raw.message);
        if (raw?.state === "passed") {
            assertion.state = "passed";
            assertion.ended = true;
        }
...
function finalizedAssertions(testState, error) {
...
function titlePathFor(context) {
    const cypressTitlePath = Cypress.currentTest?.titlePath;
    if (Array.isArray(cypressTitlePath))
        return cypressTitlePath.map(String);
    if (typeof cypressTitlePath === "function")
        return cypressTitlePath.call(Cypress.currentTest).map(String);
...
function stableId(specPath, titlePath) {
...
function installFailLensHooks() {
    if (Cypress.__failLensHooksInstalled)
        return;
    Cypress.__failLensHooksInstalled = true;
    installAssertionCapture();
    beforeEach(function failLensBeforeEach() {
        capturedAssertions = [];
        ...
        cy.task("faillens:setTest", { id: currentContext.testId, title: currentContext.title, titlePath, specPath }, { log: false });
    });
    afterEach(function failLensAfterEach() {
        const test = this.currentTest || Cypress.currentTest;
        if (!currentContext)
            return;
        const state = ["passed", "failed", "skipped"].includes(test?.state) ? test.state : "unknown";
        cy.task("faillens:setTestResult", {
