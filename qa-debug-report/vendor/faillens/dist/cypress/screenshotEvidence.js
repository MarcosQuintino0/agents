...
exports.associateScreenshots = associateScreenshots;
...
function encodedRelativePath(value) {
    return slash(value).split("/").map((segment) => segment === ".." || segment === "." ? segment : encodeURIComponent(segment)).join("/");
}
...
function positiveInteger(value) {
    const number = (0, format_1.clampNumber)(value, Number.NaN);
...
function captureScreenshotMetadata(rawDetails, context) {
    ...
    if (!href || api.isAbsolute(href))
        return undefined;
    const dimensions = (0, format_1.asRecord)(rawDetails.dimensions);
    ...
    const screenshot = {
        ...
        fileName,
        size: Math.max(0, (0, format_1.clampNumber)(rawDetails.size)),
    ...
    for (const key of ["width", "height", "takenAt", "attempt"]) {
        if (screenshot[key] === undefined)
...
function specScore(expected, actual) {
...
function attemptAt(candidate, capturedAt) {
    ...
    for (let index = attempts.length - 1; index >= 0; index -= 1) {
        ...
        const duration = (0, format_1.clampNumber)(attempt.wallClockDuration);
        if (Number.isFinite(start) && capturedAt >= start && capturedAt <= start + duration)
            return index + 1;
    }
    return undefined;
}
function associateScreenshots(candidates, captures) {
    ...
    for (const capture of captures) {
        ...
        for (const candidate of candidates) {
            ...
            if (capture.titleHint && capture.titleHint === fullTitle)
                score += 100;
            else if (capture.titleHint && capture.titleHint === shortTitle)
                score += 80;
            const timedAttempt = attemptAt(candidate, capture.capturedAt);
            if (timedAttempt)
                score += 50;
            if (capture.screenshot.kind === "failure" && candidate.state === "failed")
                score += 10;
        ...
        if (screenshot.attempt === undefined)
            delete screenshot.attempt;
    ...
    for (const screenshots of matches.values()) {
        screenshots.sort((left, right) => Number(right.kind === "failure") - Number(left.kind === "failure")
            || (right.attempt || 0) - (left.attempt || 0)
            || Date.parse(right.takenAt || "") - Date.parse(left.takenAt || ""));
    }
    return matches;
}
//# sourceMappingURL=screenshotEvidence.js.map