...
exports.extractSourceAssertions = extractSourceAssertions;
...
function firstCodeCharacter(source, start, end, expected) {
...
function titleForExpect(argumentsSource, chainSource) {
...
function targetForExpect(argumentsSource) {
    const subject = splitTopLevel(argumentsSource)[0]?.toLowerCase() || "";
    if (/\bstatus(?:code)?\b/.test(subject))
        return "status";
    if (/\bheaders?\b/.test(subject))
        return "header";
    if (/\bbody\b|response\s*$/.test(subject))
        return "body";
    return "unknown";
}
function statusExpectationFor(chainSource, title) {
...
function extractExpects(source, bodyStart, bodyEnd, file) {
    const assertions = [];
    for (let index = bodyStart; index < bodyEnd; index += 1) {
        ...
        if (source.slice(index, index + 6) !== "expect" || /[\w$]/.test(source[index - 1] || ""))
            continue;
...
function extractSourceAssertions(source, file) {
    const tests = [];
    for (let index = 0; index < source.length; index += 1) {
        const ignoredEnd = skipIgnored(source, index);
        if (ignoredEnd !== undefined) {
...
//# sourceMappingURL=extractSourceAssertions.js.map