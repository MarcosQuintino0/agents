...
exports.findImportSource = findImportSource;
exports.parseCatalogModule = parseCatalogModule;
exports.extractTestTags = extractTestTags;
...
function resolveExpr(expr, bindings) {
    ...
    if (literal && literal.end === trimmed.length && trimmed[0] !== "`")
...
function objectStringProps(objectSource) {
...
function destructuredKeys(paramSource) {
...
function classifyTags(optsSource, bindings) {
    const result = { ruleRefs: [], tags: [], catalogRefs: [] };
    ...
    for (const raw of splitTopLevel(optsSource.slice(open + 1, close))) {
        ...
        if (isString) {
            ...
            if (ruleMatch) {
                if (!ruleMatch[1].includes("${"))
                    result.ruleRefs.push(ruleMatch[1].trim());
            }
        ...
        if (member)
            result.catalogRefs.push({ object: member[1], name: member[2] });
    }
    return result;
}
function emitTest(callSource, bindings, out) {
...
function findImportSource(source, identifier) {
    const re = new RegExp(`import\\s*\\{[^}]*\\b${identifier}\\b[^}]*\\}\\s*from\\s*(['"\`])([^'"\`]+)\\1`);
...
function parseCatalogModule(source) {
    const map = new Map();
...
function scan(source, bindings, out) {
    for (let index = 0; index < source.length; index += 1) {
        const ignoredEnd = skipIgnored(source, index);
        if (ignoredEnd !== undefined) {
            index = ignoredEnd - 1;
...
function extractTestTags(source) {
    const out = [];
    scan(source, {}, out);
    return out;
}
//# sourceMappingURL=extractTestTags.js.map