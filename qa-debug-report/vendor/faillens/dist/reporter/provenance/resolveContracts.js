...
exports.resolveContracts = resolveContracts;
exports.resolveRuleRef = resolveRuleRef;
exports.contractIdForSpec = contractIdForSpec;
function mergeContract(target, incoming) {
    const comparable = (value) => JSON.stringify(Object.fromEntries(Object.entries(value).sort(([a], [b]) => a.localeCompare(b))));
    ...
    for (const file of incoming.sourceFiles)
        if (!target.sourceFiles.includes(file))
...
function resolveContracts(specs) {
    const byId = new Map();
    for (const spec of specs) {
        ...
        if (existing)
            mergeContract(existing, contract);
        else
            byId.set(contract.id, { ...contract, rules: [...contract.rules], fields: [...contract.fields] });
    }
...
function resolveRuleRef(ref, ruleIndex, preferredContractId) {
...
function contractIdForSpec(specPath, contracts) {
...
//# sourceMappingURL=resolveContracts.js.map