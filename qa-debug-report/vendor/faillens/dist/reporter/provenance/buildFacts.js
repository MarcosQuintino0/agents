...
exports.buildPersistenceState = buildPersistenceState;
exports.buildFacts = buildFacts;
...
function bodyMissingField(body, field) {
...
function responseIdentifiers(body) {
    if (!body || typeof body !== "object" || Array.isArray(body))
        return [];
    const record = body;
...
function verifiesMutation(mutation, request) {
    const generated = new Set(mutation.generatedVariables || []);
    if ((request.usedVariables || []).some((variable) => generated.has(variable)))
        return true;
...
function payloadMatches(expected, observed) {
    ...
    if (Array.isArray(expected)) {
        if (!Array.isArray(observed) || expected.length !== observed.length)
            return { matches: false, compared: true };
        let compared = false;
    ...
    for (const [key, value] of Object.entries(expected)) {
        if (!Object.prototype.hasOwnProperty.call(observed, key))
            return { matches: false, compared: true };
        const result = payloadMatches(value, observed[key]);
        if (!result.matches)
            return result;
        compared ||= result.compared;
    }
...
function buildPersistenceState(test, mainRequest, ruleRefs) {
    ...
    const expectation = resolved?.rule?.persistence
    ...
    if (mutation.method === "POST" && successful(mutation)) {
        const verification = after.find((request) => {
            if (request.method !== "GET" || !successful(request) || !verifiesMutation(mutation, request))
                return false;
    ...
    if (mutation.method === "POST" && rejected(mutation)) {
        const verification = after.find((request) => request.method === "GET" && request.receivedStatus === 404 && sameResource(mutation, request));
        return verification ? {
            expectation,
            evidence: {
...
function buildFacts(test, mainRequest, ruleRefs, maskConfig, persistence) {
    const facts = [];
    let counter = 0;
    const nextId = () => `fact-${++counter}`;
    ...
    for (const ref of ruleRefs) {
        ...
        if (rule.message) {
            facts.push({
                id: nextId(),
                kind: "rule-message",
                value: (0, sensitiveMask_1.maskSensitiveText)(rule.message, maskConfig),
                source: "contract",
                contractId: ref.contractId,
                ruleId: rule.id,
            });
...
function markConflicts(facts) {
    ...
    for (const fact of facts) {
        if (!fact.dimension)
            continue;
        if (fact.source !== "asserted" && fact.source !== "contract")
            continue;
...
//# sourceMappingURL=buildFacts.js.map