...
exports.buildBddScenario = buildBddScenario;
const node_http_1 = require("node:http");
function source(kind, id) {
    return { kind, id };
}
function line(keyword, text, sources) {
    return { keyword, text, sources };
}
function statusText(status) {
    return `HTTP ${status}${node_http_1.STATUS_CODES[status] ? ` ${node_http_1.STATUS_CODES[status]}` : ""}`;
}
...
function contractPath(method, contract) {
    const matches = (contract?.api || []).map((entry) => entry.trim().match(/^(\S+)\s+(\S+)$/))
        .filter((match) => Boolean(match && match[1].toUpperCase() === method));
...
function fieldContext(test, main, ref, contract) {
    const rule = ref?.rule;
    ...
    if (condition === "not-found") {
        ...
        if (identifier !== "")
            return [line("DADO", `que foi utilizado o identificador de teste ${shown(identifier)}`, sources)];
    }
    if (value === null)
        return [line("DADO", `que o campo "${field}" foi enviado como null`, sources)];
    if (condition === "duplicate" && comparable(value)) {
        const mainIndex = test.requests.indexOf(main);
        const prior = test.requests.slice(0, mainIndex).find((request) => isSuccess(request) && (record(request.responseBody)?.[field] === value || record(request.requestBody)?.[field] === value));
        if (prior) {
...
function authenticationContext(test, main, refs) {
    ...
    if (![expected, ...contractStatuses].some((status) => status === 401 || status === 403))
        return [];
...
function expectedLine(test, refs) {
    ...
    if (asserted !== undefined && contracted !== undefined && asserted === contracted) {
        return line("MAS", `o teste e o contrato esperavam ${statusText(asserted)}`, [assertionSource, contractSource]);
    }
    if (asserted !== undefined && contracted !== undefined) {
        return line("MAS", `o teste esperava ${statusText(asserted)} e o contrato declara ${statusText(contracted)}`, [assertionSource, contractSource]);
    }
    if (asserted !== undefined)
        return line("MAS", `o teste esperava ${statusText(asserted)}`, [assertionSource]);
    if (contracted !== undefined && contractRef?.rule) {
        return line("MAS", `a regra "${contractRef.rule.id}" declara ${statusText(contracted)}`, [contractSource]);
    }
    return undefined;
}
function persistenceLine(test) {
    const evidence = test.persistenceEvidence;
    if (!evidence || evidence.state === "not-verified" || !evidence.verificationRequestId)
        return undefined;
    const messages = {
...
function buildBddScenario(test, main, refs, contract) {
    if (test.state !== "failed" || !main)
        return undefined;
    ...
    if (contractPath(main.method, contract) && contract)
        operationSources.push(source("contract", contract.id));
    ...
    const selectedContext = context.slice(0, availableContext).map((item, index) => ({ ...item, keyword: index === 0 ? "DADO" : "E" }));
    const lines = [...selectedContext, ...core];
    return { lines, text: lines.map((item) => `${item.keyword} ${item.text}`).join("\n") };
}
//# sourceMappingURL=buildBddScenario.js.map