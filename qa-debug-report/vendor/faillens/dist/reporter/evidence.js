...
exports.sanitizeEvidence = sanitizeEvidence;
exports.buildIssueContent = buildIssueContent;
exports.buildEvidenceText = buildEvidenceText;
exports.buildEvidenceHtml = buildEvidenceHtml;
function validRelativePath(value) {
    if (typeof value !== "string" || !value || value.includes("\0") || value.includes("\\"))
        return undefined;
    if (value.startsWith("/") || /^[A-Za-z]:/.test(value) || !/\.png$/i.test(value))
        return undefined;
    const parts = value.split("/");
    return parts.some((part) => !part || part === "." || part === "..") ? undefined : value;
}
function validHref(value, relativePath) {
    if (typeof value !== "string" || !value || value.includes("\0") || value.includes("\\"))
        return undefined;
    ...
    try {
        for (const part of parts) {
            ...
            if (!item || item === "." || item.includes("/") || item.includes("\\") || item.includes("\0"))
                return undefined;
            decoded.push(item);
        }
    }
...
function sanitizeEvidence(value) {
    ...
    for (const source of value.screenshots) {
        ...
        if (!href || source.fileName !== fileName || (source.kind !== "failure" && source.kind !== "manual"))
            continue;
        ...
        if (Number.isFinite(source.width) && Number(source.width) > 0)
            screenshot.width = Math.round(Number(source.width));
        if (Number.isFinite(source.height) && Number(source.height) > 0)
            screenshot.height = Math.round(Number(source.height));
        if (source.takenAt && Number.isFinite(Date.parse(source.takenAt)))
            screenshot.takenAt = new Date(source.takenAt).toISOString();
        if (Number.isInteger(source.attempt) && Number(source.attempt) > 0)
            screenshot.attempt = Number(source.attempt);
        screenshots.push(screenshot);
    }
    return screenshots.length ? { screenshots } : undefined;
}
function buildIssueContent(test, specPath, contracts = []) {
    ...
    const status = (value) => `${value}${reasons[value] ? ` ${reasons[value]}` : ""}`;
    const pathOf = (url) => {
        try {
    ...
    const contractPaths = (contract?.api || []).map((entry) => entry.trim().match(/^(\S+)\s+(\S+)$/))
        .filter((match) => Boolean(match && match[1].toUpperCase() === main.method));
    ...
    const responseRecord = main.responseBody && typeof main.responseBody === "object" && !Array.isArray(main.responseBody)
        ? main.responseBody
        : undefined;
    const identifier = responseRecord && Object.entries(responseRecord).find(([key, value]) => /(^id$|id$)/i.test(key) && (typeof value === "string" || typeof value === "number"));
    if (identifier)
        currentResult += ` A resposta retornou o identificador ${JSON.stringify(identifier[1])}.`;
    ...
    for (const rule of rules) {
        if (rule?.message)
            expectedParts.push(`Mensagem declarada pela regra "${rule.id}": ${rule.message}`);
        if (rule?.persistence === "forbidden")
            expectedParts.push("A regra contratual determina que a operação rejeitada não persista alterações.");
        if (rule?.persistence === "preserve")
            expectedParts.push("A regra contratual determina que o estado anterior seja preservado.");
        if (rule?.persistence === "remove")
            expectedParts.push("A regra contratual determina que o recurso seja removido.");
    }
    ...
    for (const fact of test.facts || []) {
        if (fact.kind !== "request-field-absent" || typeof fact.value !== "string")
            continue;
...
function buildEvidenceText(input) {
    ...
    const headers = (value) => Object.entries(value)
        .map(([key, item]) => `${oneLine(key)}: ${oneLine(item)}`).join("\n") || "(nenhum header capturado)";
    const cell = (value) => `\`${oneLine(value).replace(/`/g, "'").replace(/\|/g, "\\|")}\``;
    const bdd = typeof input.bdd === "string"
        ? input.bdd.split(/\r?\n/).map(oneLine).filter(Boolean).join("\n")
        : "";
    const comparison = input.comparison.length
        ? ["| Validação | Esperado | Recebido |", "|---|---|---|", ...input.comparison.map((row) => `| ${oneLine(row.label).replace(/\|/g, "\\|")} | ${cell(row.expected)} | ${cell(row.received)} |`)]
        : ["Nenhuma comparação adicional disponível."];
    const trace = ["| Informação | Valor |", "|---|---|", ...input.traceability.map((row) => `| ${oneLine(row.label).replace(/\|/g, "\\|")} | ${cell(row.value)} |`)];
    return [
...
function buildEvidenceHtml(input, imageDataUrl) {
    ...
    const comparison = input.comparison.length
        ? `<table><thead><tr><th>Validação</th><th>Esperado</th><th>Recebido</th></tr></thead><tbody>${input.comparison.map((row) => `<tr><td>${escape(row.label)}</td><td><code>${escape(row.expected)}</code></td><td><code>${escape(row.received)}</code></td></tr>`).join("")}</tbody></table>`
        : "<p>Nenhuma comparação adicional disponível.</p>";
    const trace = `<table><thead><tr><th>Informação</th><th>Valor</th></tr></thead><tbody>${input.traceability.map((row) => `<tr><td>${escape(row.label)}</td><td><code>${escape(row.value)}</code></td></tr>`).join("")}</tbody></table>`;
