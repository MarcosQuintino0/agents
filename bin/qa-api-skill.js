...
const scriptUrl = (name) => pathToFileURL(path.join(sourceSkill, "scripts", name)).href;
...
const {
  ...
  installGraphify,
  updateGraph,
} = await import(scriptUrl("graphify-manager.js"));
const {
  explainNode,
  findPath,
  queryGraph,
} = await import(scriptUrl("graph-query.js"));
const { provideGraphEvidence } = await import(scriptUrl("graph-provider.js"));
